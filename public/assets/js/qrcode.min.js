/**
 * @fileoverview QRCode.js
 * @author davidshimjs
 * @see http://www.davidshimjs.com/qrcode
 */
var QRCode;
...
(function () {
	...
	//   http://www.denso-wave.com/qrcode/faqconstant.html
	//---------------------------------------------------------------------
	...
	var QRUtil = {
		...
		getMask: function (maskPattern, row, col) {
			switch (maskPattern) {
				...
				case QRMaskPattern.PATTERN111: return ((row + col) % 2 + (row * col) % 3) % 2 == 0;
	...
	for (var i = 8; i < 256; i++) QRMath.EXP_TABLE[i] = QRMath.EXP_TABLE[i - 4] ^ QRMath.EXP_TABLE[i - 5] ^ QRMath.EXP_TABLE[i - 6] ^ QRMath.EXP_TABLE[i - 1];
	...
	function QRPolynomial(num, shift) {
		...
		for (var k = 0; k < this.num.length; k++) this.num[k] = 0;
	...
	QRPolynomial.prototype = {
		...
		mod: function (e) {
			...
			var eLen = e.getLength();
			while (num.length >= eLen) {
				if (num[0] === 0) {
					num.shift();
					continue;
				}
	...
	QRRSBlock.RS_BLOCK_TABLE = [
		...
		[13, 145, 115, 3, 146, 116], [2, 74, 46, 27, 75, 47], [42, 54, 24, 1, 55, 25], [23, 45, 15, 28, 46, 16],
		[17, 145, 115], [10, 74, 46, 23, 75, 47], [10, 54, 24, 35, 55, 25], [19, 45, 15, 35, 46, 16],
		[17, 145, 115, 1, 146, 116], [14, 74, 46, 21, 75, 47], [29, 54, 24, 19, 55, 25], [11, 45, 15, 46, 46, 16],
		[13, 145, 115, 6, 146, 116], [14, 74, 46, 23, 75, 47], [44, 54, 24, 7, 55, 25], [59, 46, 16, 1, 47, 17],
		[12, 151, 121, 9, 152, 122], [12, 75, 47, 26, 76, 48], [39, 54, 24, 14, 55, 25], [22, 45, 15, 41, 46, 16],
		[6, 151, 121, 16, 152, 122], [6, 75, 47, 34, 76, 48], [46, 54, 24, 10, 55, 25], [2, 45, 15, 64, 46, 16],
		[17, 152, 122, 6, 153, 123], [29, 74, 46, 14, 75, 47], [49, 54, 24, 10, 55, 25], [24, 45, 15, 46, 46, 16],
		[4, 152, 122, 21, 153, 123], [13, 74, 46, 32, 75, 47], [48, 54, 24, 14, 55, 25], [42, 45, 15, 32, 46, 16],
	...
	// Dynamic auto type number resolution
	...
	function _getUTF8Length(sText) {
		var replaceText = sText.replace(/[\u0080-\u07ff]/g, '..').replace(/[\u0800-\uffff]/g, '...');
	...
	// Public QRCode class (David Shim API)
	...
	QRCode.prototype.makeCode = function (sText) {
		...
		if (this._el) {
			...
			var svg = '<svg xmlns="http://www.w3.org/2000/svg" width="' + size + '" height="' + size + '" viewBox="0 0 ' + nCount + ' ' + nCount + '" shape-rendering="crispEdges" style="display:block; width:' + size + 'px; height:' + size + 'px;">';
			svg += '<rect width="' + nCount + '" height="' + nCount + '" fill="' + this._htOption.colorLight + '"/>';
			for (var row = 0; row < nCount; row++) {
				for (var col = 0; col < nCount; col++) {
					if (this._oQRCode.isDark(row, col)) {
						svg += '<rect x="' + col + '" y="' + row + '" width="1" height="1" fill="' + this._htOption.colorDark + '"/>';
					}
				}
			}
			svg += '</svg>';
	...
	// Backward compatibility helper for legacy call signature qrcode(0, 'M')
	window.qrcode = function (typeNumber, errorCorrectionLevel) {
		var textData = "";
		return {
			addData: function (data) { textData = data; },
			make: function () {},
			createSvgTag: function (cellSize, margin) {
				var dummyContainer = document.createElement("div");
				var qr = new QRCode(dummyContainer, { text: textData, width: 140, height: 140 });
